...
class InterviewDataProcessor:
    ...
    def clean_dataset(self):
        ...
        self.df = self.df[self.df['candidate_answer'].apply(
            DataValidator.validate_answer_length
        )]
    ...
    def evaluate_all_answers(self):
        ...
        self.df['semantic_similarity_percent'] = (
                self.df['semantic_similarity'] * 100
        ).round(1)
        return self.df
    ...
    def get_dataset_info(self):
        if self.df is None:
            raise ProcessingError("Dataset not loaded")

        return {
            "rows": self.df.shape[0],
            "columns": self.df.shape[1],
            "roles": self.df['role'].unique().tolist(),
            "questions": self.df['question'].nunique(),
